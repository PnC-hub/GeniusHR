...
export async function GET() {
  try {
    ...
    // Return mock data
    return NextResponse.json(mockMansionari)
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    if (!title || !ccnlLevel || !department) {
      return NextResponse.json({
        error: 'Titolo, livello CCNL e reparto sono obbligatori'
      }, { status: 400 })
    }

    // Mock created mansionario
    const newMansionario = {
      id: String(mockMansionari.length + 1),
      title,
      ccnlLevel,
      department,
      status: 'draft',
    ...
    return NextResponse.json(newMansionario, { status: 201 })
  } catch (error) {
    console.error('Error creating mansionario:', error)
